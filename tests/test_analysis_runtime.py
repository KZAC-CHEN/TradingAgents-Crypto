...
class _FakeGraph:
    ...
    def resolve_instrument_context(self, symbol, asset_type):
        return f"{asset_type}:{symbol}"

    def begin_checkpoint(self, symbol, analysis_date, asset_type):
        return "checkpoint-1"
...
def test_runner_emits_events_and_writes_reports(tmp_path: Path) -> None:
    ...
    assert graph.clear_calls == [("BTC-USD", "2026-09-19", "crypto")]
